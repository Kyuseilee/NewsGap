...
export class MobileRSSFetcher {
   ...
   * 从单个 RSS 源爬取文章
   */
  async fetchFromSource(source: RSSSource, hoursAgo?: number): Promise<Article[]> {
    try {
  ...
  async fetchMultipleSources(
    ...
    const results = await Promise.allSettled(
      sources.map(source => this.fetchFromSource(source, hoursAgo))
    );
   ...
   * 按行业爬取（从内置配置）
   */
  async fetchByIndustry(industry: string, hoursAgo: number = 24): Promise<Article[]> {
   ...
   */
  private convertToArticle(item: any, source: RSSSource): Article {
   ...
   * 生成文章 ID
   */
  private generateArticleId(uniqueString: string): string {
    // 简单的哈希函数
    let hash = 0;
   ...
   * 获取内置信息源配置（按行业）
   */
  private getSourcesByIndustry(industry: string): RSSSource[] {
    const allSources: Record<string, RSSSource[]> = {
      tech: [
        {
          id: 'tech_36kr',
          name: '36氪',
