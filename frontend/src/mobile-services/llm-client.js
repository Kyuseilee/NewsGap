...
export class MobileLLMClient {
     ...
     * 分析文章生成情报报告
     */
    async analyze(request, articles) {
     ...
     * Gemini API 调用
     */
    async analyzeWithGemini(articles, model, apiKey, customPrompt) {
     ...
     * DeepSeek API 调用
     */
    async analyzeWithDeepSeek(articles, model, apiKey, customPrompt) {
        const prompt = this.buildPrompt(articles, customPrompt);
        const response = await Http.post({
     ...
     * OpenAI API 调用
     */
    async analyzeWithOpenAI(articles, model, apiKey, customPrompt) {
        const prompt = this.buildPrompt(articles, customPrompt);
        const response = await Http.post({
     ...
     * 构建分析 Prompt
     */
    buildPrompt(articles, customPrompt) {
